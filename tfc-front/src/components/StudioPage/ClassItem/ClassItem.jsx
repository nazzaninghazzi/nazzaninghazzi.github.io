...
export default function ClassItem(props) {
    ...
    useEffect(() => {
        ...
        classes.forEach((classItem) => {
            if (classItem.class_instance_id === props.classItem.id) {
    ...
    return (
        <div className="studio-page-class-item">
            <h3 className='no-margin-padding lignt-color'>{props.classItem.name}</h3>
            <p className='no-margin-padding lignt-color'><b>Description:</b> {props.classItem.description}</p>
            <p className='no-margin-padding lignt-color'><b>Date:</b> {new Date(props.classItem.date + ' ' + props.classItem.start_time).toLocaleString()}</p>
            <p className='no-margin-padding lignt-color'><b>Duration:</b> {props.classItem.length} minutes</p>
            <p className='no-margin-padding lignt-color'><b>Coach:</b> {props.classItem.coach}</p>
            <p className='no-margin-padding lignt-color'><b>Space Left:</b> {props.classItem.capacity - props.classItem.num_signed_up}/{props.classItem.capacity}</p>
            <Button
